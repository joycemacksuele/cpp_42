...
/*   Updated: 2022/09/27 11:50:12 by jfreitas      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
	...
	while (1) {
		...
		//	std::cout << std::endl;
			...
			bool isSigned = false;
			int i = 4;
			while (isSigned == false && i >= 0) {
				if (i == 0) {
					//std::cout << std::endl << YELLOW << "You have no tries left. Bye!" << RESET << std::endl << std::endl;
					throw -1;
				}
				i--;
				...
				try {
					...
					std::cerr << std::endl << BOLD RED "Error: " RESET BOLD << e << RESET << std::endl << std::endl;
					if (i > 0) {
						std::cerr << "Try again " <<  YELLOW << "(you have " << std::to_string(i) << " tries left)." << RESET << std::endl << std::endl;
					}
					continue ;
				}
			...
			sleep(1);
			std::cerr << std::endl << YELLOW << "You have no tries left! Bye." << RESET << std::endl << std::endl;
			break ;
		}
